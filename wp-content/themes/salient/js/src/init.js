...
(function($, window, document) {
	...
	"use strict";
		...
		*/
		...
		NectarFlickity.prototype.init = function() {
			...
			// Add live region for slide change announcements (must exist before events() binds change handler).
			// Skip for tickers — continuous auto-rotation would spam screen readers.
			if (!this.ticker) {
				// Remove any prior live region from a previous init to avoid duplicates.
				this.$el.children('.nectar-slide-live').remove();
				this.$liveRegion = $('<div class="screen-reader-text nectar-slide-live" aria-live="polite" aria-atomic="true"></div>');
				this.$el.append(this.$liveRegion);
			}

			// Add ARIA attributes to slides.
			var $cells = this.$el.find('.flickity-slider').children();
			var totalCells = $cells.length;
			var slideRoleDesc = (window.nectar_front_i18n && nectar_front_i18n.slide) ? nectar_front_i18n.slide : 'slide';
			$cells.each(function(i) {
				$(this).attr({
					'role': 'group',
					'aria-roledescription': slideRoleDesc,
					'aria-label': (i + 1) + ' / ' + totalCells
				});
			});
			...
			if(this.$el.is('[data-overflow="visible"]') &&
				 this.$el.parents('.wpb_column:not(.vc_col-sm-12)').length == 0 ) {
					...
					if( this.$el.parents('.wpb_row:not(.nectar-overflow-hidden):not(.full-width-content):not(.inner_row):not(.has-global-section):not(.nectar_section)').length > 0 ) {
						this.$el.wrap('<div class="wpb_row vc_row-fluid vc_row full-width-content nectar-overflow-x-clipped carousel-dynamic-wrapper"><div class="normal-container container"></div></div>');
						...
						this.$el.parents('.wpb_row.full-width-content:not(.nectar-overflow-hidden)').addClass('nectar-overflow-x-clipped');
					}
		...
		NectarFlickity.prototype.sharedEvents = function() {
			...
			if (!$that.attr('tabindex')) {
				$that.attr('tabindex', '0');
			}
			$that.on('keydown', function(e) {
				// Only respond when the carousel container itself is focused — prevents
				// hijacking arrow keys from focused descendants (tabs, links, form fields, nested carousels).
			...
			if (pointer.$liveRegion) {
				flickityInstance.on('change', function(index) {
					var total = flickityInstance.slides ? flickityInstance.slides.length : 0;
					if (total > 0) {
						var msg = (window.nectar_front_i18n && nectar_front_i18n.slide_of) ? nectar_front_i18n.slide_of.replace('%1$s', index + 1).replace('%2$s', total) : (index + 1) + ' / ' + total;
						pointer.$liveRegion.text(msg);
					}
				});
			}
	};
    ...
    function clamp(val,min, max) {
		...
		*/
		function fancyBoxInit() {
			...
			$("[data-fancybox]").fancybox({
				...
				hash: false,
				beforeShow: function() {
					$(window).trigger('nectar-fancybox-open');
				},
				afterClose: function() {
					$(window).trigger('nectar-fancybox-close');
				},
		...
		*/
		function tabbedChangeSection(clickedTab) {
			...
			if (!clickedTab.hasClass('active-tab') && !clickedTab.hasClass('loading')) {
				...
				if (clickedTab.is('[role="tab"]')) {
					clickedTab.closest('[role="tablist"]').find('> li > a[role="tab"]').attr('aria-selected', 'false').attr('tabindex', '-1');
			...
			// Fix columns inside tabs - batch reads then writes to avoid forced reflow per column
			clickedTab.parents('.tabbed').find('.wpb_row').each(function () {
				var $cols = $(this).find('[class*="vc_col-"]');
				if ($cols.length === 0) {
					return;
				}

				// Phase 1: batch all layout reads (single layout flush per row)
				var firstLeft = $cols[0].getBoundingClientRect().left;
				var lefts = new Array($cols.length);
				for (var i = 0; i < $cols.length; i++) {
					lefts[i] = $cols[i].getBoundingClientRect().left;
				}

				// Phase 2: batch all DOM writes
				var threshold = firstLeft + 15;
				for (var j = 0; j < $cols.length; j++) {
					if (lefts[j] < threshold) {
						$cols[j].classList.add('no-left-margin');
					} else {
						$cols[j].classList.remove('no-left-margin');
					}
				}
			});
		...
		*/
		function tabbedInit() {
			...
			$body.on('keydown', '.wpb_tabs_nav[role="tablist"] a[role="tab"]', function(e) {
				var $tablist = $(this).closest('[role="tablist"]');
				// Get tabs only in this tablist (handles nested tabs and the menu-content wrapper in sticky tabs).
				var $tabs = $tablist.find('a[role="tab"]').filter(function() {
					return $(this).closest('[role="tablist"]')[0] === $tablist[0];
				});
			...
			$body.on('click', '.nectar-sticky-tabs .wpb_tabs_nav a', function (e) {
				...
				if (!clickedTab.parents('li').hasClass('active-tab') && !tabAnimating || !$parentTabs.hasClass('loaded')) {
					...
					// Update ARIA states for sticky tabs.
					if (clickedTab.is('[role="tab"]')) {
						$parentTabs.find('.wpb_tabs_nav a[role="tab"]').attr('aria-selected', 'false').attr('tabindex', '-1');
						clickedTab.attr('aria-selected', 'true').attr('tabindex', '0');
					}
	...
	VerticalScrollingTabs.prototype.lineAnimation = function () {
		...
		*/
		function accordionInit() {
			...
			setTimeout(function() {
				...
				$('.toggles.accordion:not([data-starting="closed"])').each(function () {
					...
					// Set aria-expanded on the initially open toggle.
					$(this).find('> .toggle').first().find('.toggle-title a[aria-expanded], > h3 a[aria-expanded]').attr('aria-expanded', 'true');
		...
		function accordionClickEvents() {
			...
			$body.on('click', '.toggle > h3 a, .toggle > .toggle-title a', function () {
				...
				if (!$(this).parents('.toggles').hasClass('accordion')) {
					...
					$(this).attr('aria-expanded', $(this).parents('.toggle').hasClass('open') ? 'true' : 'false');
			...
			$body.on('click', '.toggles.accordion .toggle > h3 a, .toggles.accordion .toggle > .toggle-title a', function (e) {
				...
				if ($(this).parents('.toggle').hasClass('open')) {
					...
					$parentToggles.find('.toggle .toggle-title a[aria-expanded], .toggle > h3 a[aria-expanded]').attr('aria-expanded', 'false');
					return false;
				}
				...
				if( $(this).parents('.toggle[data-inner-wrap="true"]').length == 0 ) {
					...
					$parentToggles.find('.toggle h3 a i').attr('class', 'fa fa-plus-circle');
					$parentToggles.find('.toggle').removeClass('open');
					// Reset ARIA for legacy accordion.
					$parentToggles.find('.toggle .toggle-title a[aria-expanded], .toggle > h3 a[aria-expanded]').attr('aria-expanded', 'false');
				...
				// Update ARIA expanded states for accordion.
				$parentToggles.find('.toggle .toggle-title a[aria-expanded], .toggle > h3 a[aria-expanded]').attr('aria-expanded', 'false');
				if ($parentToggle.hasClass('open')) {
		...
		// Toggle Space key support (role="button" expects Space activation).
		$body.on('keydown', '.toggle > .toggle-title a[role="button"], .toggle > h3 a[role="button"]', function(e) {
			if (e.keyCode === 32) {
				e.preventDefault();
				$(this).trigger('click');
			}
				...
				*/
				function fullWidthContentColumns() {
					...
					$('.main-content > .row > ' + $frontEndEditorElDiv + ' .full-width-content, #portfolio-extra > ' + $frontEndEditorElDiv + ' .full-width-content, .woocommerce-tabs #tab-description > .full-width-content, .post-area.span_12 article .content-inner > .full-width-content').each(function () {
						...
						// Skip rows with auto equal height disabled.
						if ($(this).hasClass('no-auto-equal-height')) {
							return;
						}
			...
			function nectarPostGridInit() {
				...
				*/
				function videoBGInit() {
					...
					// iOS Low Power Mode blocks autoplay even on muted videos and
					// overlays a play button on the <video>. These are background-
					// only elements behind content, so hide the wrap and let the
					// .mobile-video-image poster / background color show instead.
					// Only hides on explicit NotAllowedError. Videos already
					// processed by nectar-delay-javascript.js are skipped via the
					// data-nectar-autoplay-checked flag to prevent double-handling.
					$('.nectar-video-wrap video.nectar-video-bg').each(function() {

						var video = this;

						if (video.dataset.nectarAutoplayChecked === 'true') {
							return;
						}
						video.dataset.nectarAutoplayChecked = 'true';

						var $wrap = $(video).closest('.nectar-video-wrap');

						var hideWrap = function() {
							if ($wrap.data('autoplayHidden')) {
								return;
							}

							// Only hide when a real poster-image fallback exists.
							// Without one, the user would see nothing instead of
							// the (ugly but informative) play button.
							var fallback = $wrap.parent().children('.mobile-video-image')[0];
							if (!fallback) {
								return;
							}
							var bg = window.getComputedStyle(fallback).backgroundImage;
							if (!bg || bg === 'none' || /^url\(["']?["']?\)$/.test(bg)) {
								return;
							}

							$wrap.data('autoplayHidden', true);
							$(fallback).show();
							// opacity:0 (not display:none) preserves the wrap's
							// dimensions for any parallax/observer code. The wrap
							// is position:absolute so flow is unaffected either way.
							$wrap.css({
								'opacity': 0,
								'pointer-events': 'none'
							});
            ...
            $('#slide-out-widget-area .off-canvas-menu-container li').on('mouseleave',function() {
				...
				*/
				...
				NectarWooCommerceQuantity.prototype.updateCart = function(val) {
					...
					// When removeFlag is set, send quantity 0 so the server removes the item.
					if ( that.state.removeFlag === true ) {
						val = 0;
					}
				...
				function letterRevealMarkup($el) {
					...
					var animatedClass = ( $el.hasClass('animated-in') ) ? ' animated' : '';
						...
						$el.find('> *').each(function () {
							...
							for (var i = 0; i < textArr.length; i++) {
								$(this)[0].innerHTML += '<span aria-hidden="true"><span class="inner'+animatedClass+'">' + textArr[i] + '</span></span> ';
							}
				...
				function splitLineHeadingMarkup() {
					...
					$('.nectar-split-heading[data-animation-type="line-reveal-by-space"][data-text-effect*="letter-reveal"]').each(function () {
						...
						$(this).find('> *').each(function () {
							...
							for (var i = 0; i < textArr.length; i++) {
								$(this)[0].innerHTML += '<span aria-hidden="true">' + textArr[i] + '</span> ';
							}
              ...
              selfHostedVideoObserver.observe($(this)[0]);
					...
					if ($('#nectar_fullscreen_rows').length == 0 || $nectarFullPage.$usingFullScreenRows == false) {
						...
						if ($('#slide-out-widget-area .off-canvas-menu-container').length > 0) {
							$('#slide-out-widget-area .off-canvas-menu-container').find("a[href*='" + location.pathname + "']").each(function () {
								...
								// Guard against malformed fragments (e.g. IDs starting with a digit) throwing in Sizzle.
								var $hashDivs = $();
								try { $hashDivs = $('div' + $href.substr($href.indexOf("#"))); } catch(e) {}
								...
								var $hashFullpage = $();
								try { $hashFullpage = $('div[data-fullscreen-anchor-id="' + $href.substr($href.indexOf("#") + 1) + '"]'); } catch(e) {}
								if ($hashFullpage.length > 0) {
				...
				*/
				function pageLoadHash() {
					...
					// Guard against malformed hashes (e.g. IDs starting with a digit) throwing in Sizzle.
					var $hashInMain = $();
					try { if ($hash) { $hashInMain = $('.main-content').find($hash); } } catch(e) {}

					if ($hash && $hashInMain.length > 0 ||
					$hash && $('.main-content').find('[data-fullscreen-anchor-id="' + $hashSubstrng + '"]').length > 0) {

						var $hashObj    = ($hashInMain.length > 0) ? $hashInMain : $('.main-content').find('[data-fullscreen-anchor-id="' + $hashSubstrng + '"]'),
						...
						setTimeout(function () {
							...
							if( headerIsContained === true ) {
								...
								$scrollTopDistance = $hashObj.offset().top - containedHeaderHeight - nectarDOMInfo.adminBarHeight;
							}
						...
						$('select:not(.comment-form-rating #rating)').each(function () {
							...
							//default
								...
								if ($(this).parents('#buddypress').length == 0 &&
										...
										$(this).parents('.yith_wccl_is_custom').length == 0) {
									//if label is found
