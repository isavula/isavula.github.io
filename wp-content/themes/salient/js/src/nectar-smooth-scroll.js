...
(function(){
  ...
  var usingSmoothScroll = (window.nectarOptions && window.nectarOptions.smooth_scroll && window.nectarOptions.smooth_scroll === 'true') ? true : false;
  var smoothScrollStrength = (window.nectarOptions && window.nectarOptions.smooth_scroll_strength) ? window.nectarOptions.smooth_scroll_strength : '50';
  smoothScrollStrength = parseInt(smoothScrollStrength);
  ...
  if ( window.jQuery ) {
    ...
    jQuery(window).on('nectar-fancybox-open', function() {
      if ( lenis ) {
        lenis.stop();
      }
    });
    jQuery(window).on('nectar-fancybox-close', function() {
      if ( lenis ) {
        lenis.start();
      }
    });
